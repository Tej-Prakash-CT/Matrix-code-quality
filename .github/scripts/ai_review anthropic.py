...
def review_finding(client: "anthropic.Anthropic", finding: dict, snippet: str) -> dict:
    prompt = (
...
def pr_summary(client: "anthropic.Anthropic", report: dict) -> str:
...
def main() -> None:
    ...
    if not api_key:
        print("[WARN] ANTHROPIC_API_KEY not set — writing empty ai-review.json")
        Path(OUTPUT).write_text(json.dumps({**empty, "error": "ANTHROPIC_API_KEY not set"}, indent=2))
        return
    ...
    print(f"[AI] {len(findings)} findings to review")
    ...
    for i, finding in enumerate(findings, 1):
        ...
        reviewed.append({**finding, "ai": review_finding(client, finding, snippet)})

    print("[AI] Generating PR summary...")
    summary = pr_summary(client, report)
